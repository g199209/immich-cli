...
assets are detected and skipped.
...
```
—generated by immich-cli/v1 sha=kL2A3+Icry1a
```
...
The API token in `[immich]` must have the **`asset.update`** scope, or
every PUT comes back 403. Create a fresh token in Immich's Web UI →
